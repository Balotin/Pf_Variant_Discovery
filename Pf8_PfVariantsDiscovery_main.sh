...
# Change the config according to the node reserved: (srun -p normal -c 20 --pty bash -i)
###### SLURM CONFIGURATION ########
#SBATCH --job-name=PfVariants
#SBATCH --output=PfVariants_out_MADBIO.txt
#SBATCH --error=PfVariants_err_MADBIO.txt
#SBATCH --ntasks=1
...
#SBATCH --mail-user=balotin.fogang@umontpellier.fr
###################################

set -euo pipefail

# 1. Clean and activate conda environment
module purge 

echo "activate conda env"
source /usr/local/bioinfo/Miniforge3-24.9.2-0/etc/profile.d/conda.sh
conda activate /home/fogang/.conda/envs/Pf-discovery
echo "Conda env: $CONDA_DEFAULT_ENV"

# 3. Create job-specific scratch directory
SCRATCH_DIR="/scratch/fogang/fogang-$SLURM_JOB_ID"
mkdir -p $SCRATCH_DIR
cd $SCRATCH_DIR

# 4. Transfer data from SAN to local scratch (fast access)
scp -r san:/projects/medium/GATACmalaria/MADBIO/sWGS/Pf_Variants_Discovery .     #TOBEMODIFIED

cd Pf_Variants_Discovery                 #TOBEMODIFIED
...
SAMPLE_LIST="$BASEDIR/data/sample_list.txt"                          #TOBEMODIFIED
GVCF_SAMPLE_LIST="$BASEDIR/data/sample_list.txt"                        #TOBEMODIFIED
REFGENOME_DIR="$BASEDIR/data/Reference"
REFGENOME="$BASEDIR/data/Reference/Pfalciparum.genome.fasta"                #TOBEMODIFIED

KNOWN_SITES="$BASEDIR/data/Useful_files/KnownSites_Pf3D7_AllChr_v3_PASS_only.pf7.vcf.gz"
GFF_PF7="$BASEDIR/data/Reference/Pfalciparum_replace_Pf3D7_MIT_v3_with_Pf_M76611.gff"   #TOBEMODIFIED
DICT="$BASEDIR/data/Reference/Pfalciparum.genome.dict"                                   #TOBEMODIFIED
RAWREADS="$BASEDIR/data/reads"                                                          #TOBEMODIFIED
Adapters="$BASEDIR/data/Adaptators_seq/TruSeq3-PE.fa"                                    #TOBEMODIFIED                                    
...
threads=20         #TOBEMODIFIED
batch_size=50        #TOBEMODIFIED
interval_padding=500   #TOBEMODIFIED
contamination=0         #TOBEMODIFIED
...
# 6. Transfer selected results to permanent storage
cd ..
cd $SCRATCH_DIR/Pf_Variants_Discovery
DEST_DIR="san:/projects/medium/GATACmalaria/MADBIO/sWGS/Pf_Variants_Discovery/MADBIO_sWGA_PfVariantsDiscovery_output" # TOBEMODIFIED

# Only copy desired result folders
scp -r "$MAPDIR" "$DEST_DIR"
scp -r "$VCF_SNP_INDELS" "$DEST_DIR"
#scp -r "$TRIMDIR" "$DEST_DIR"
